...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 19 20:27:14 2017
...
Tf = 24 * 1
Dt=24 * 1
times = np.arange(Dt*HOURS_TO_DAY, (Dt + Tf) * HOURS_TO_DAY,  1* HOURS_TO_DAY)
...
SimC.ACTIONS = tuple(np.arange(0,360,1)) 
...
mt.UCT_COEFF=1/2
stateInit = [0, 47.5, -3.5 + 360]
SimC.ACTIONS = tuple(np.arange(0,360,45)) 
Sim = Simulator(times, lats, lons, Wavg, Wspr, stateInit)
...
name=coeff+'UCT_theTree_heading_'+str(heading)+'_b'+str(tree.budget)+'_'+str(Tf/24)+'days_1dayshiftedHalfReducedDistance'
#%%
...
# plt.ylabel('Probability distribution function [s/m]')
# plt.show()
